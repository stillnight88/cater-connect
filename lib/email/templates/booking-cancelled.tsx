import {
    Body,
    Container,
    Head,
    Heading,
    Html,
    Preview,
    Section,
    Tailwind,
    Text,
} from '@react-email/components';

interface BookingCancelledTemplateData {
    recipientName: string;
    cancelledByName: string;
    cancelledBy: 'customer' | 'vendor';
    eventDate: string;
    eventAddress: string;
}

export function generateBookingCancelledTemplate({
    recipientName,
    cancelledByName,
    cancelledBy,
    eventDate,
    eventAddress,
}: BookingCancelledTemplateData) {
    const cancelledByLabel = cancelledBy === 'customer' ? 'customer' : 'vendor';

    return (
        <Html>
            <Head />
            <Preview>A booking for {eventDate} has been cancelled</Preview>

            <Tailwind>
                <Body className="bg-[#f5f5f5] m-0 font-sans">
                    <Container className="max-w-150 mx-auto my-10 bg-white rounded-[8px] overflow-hidden">
                        {/* HEADER */}
                        <Section className="bg-[#6c757d] py-10 px-5 text-center">
                            <Heading className="text-white text-[24px] font-semibold m-0">
                                Booking Cancelled
                            </Heading>
                        </Section>

                        {/* CONTENT */}
                        <Section className="px-7.5 py-10 text-[#333]">
                            <Text className="text-[18px] mb-5">
                                Hi {recipientName},
                            </Text>

                            <Text className="text-[16px] leading-6 mb-5">
                                The booking scheduled for{' '}
                                <strong>{eventDate}</strong> has been cancelled
                                by the {cancelledByLabel}{' '}
                                <strong>{cancelledByName}</strong>.
                            </Text>

                            {/* BOOKING DETAILS */}
                            <Section className="bg-[#e2e3e5] border-l-4 border-[#6c757d] px-3.75 py-3.75 my-5">
                                <Text className="text-[14px] text-[#383d41] m-0 font-semibold">
                                    Cancelled Booking Details
                                </Text>
                                <Text className="text-[14px] text-[#383d41] m-0">
                                    • <strong>Event Date:</strong> {eventDate}
                                </Text>
                                <Text className="text-[14px] text-[#383d41] m-0">
                                    • <strong>Location:</strong> {eventAddress}
                                </Text>
                                <Text className="text-[14px] text-[#383d41] m-0">
                                    • <strong>Cancelled by:</strong>{' '}
                                    {cancelledByLabel} ({cancelledByName})
                                </Text>
                            </Section>

                            <Text className="text-[16px] leading-6">
                                If you have any questions about this
                                cancellation, please contact our support team.
                            </Text>
                        </Section>

                        {/* FOOTER */}
                        <Section className="bg-[#f8f9fa] px-7.5 py-7.5 text-center border-t border-[#e9ecef]">
                            <Text className="text-[14px] text-[#666] m-0">
                                Questions? Contact us at support@caterconnect.com
                            </Text>
                            <Text className="text-[12px] text-[#999] mt-5">
                                © {new Date().getFullYear()} CaterConnect. All
                                rights reserved.
                            </Text>
                        </Section>
                    </Container>
                </Body>
            </Tailwind>
        </Html>
    );
}